...
namespace SlideShowPro.Api
{
    internal class JsonDeserializerWithEpochDates : IDeserializer
	{
        ...
        public JsonDeserializerWithEpochDates()
		{
			...
			foreach (var prop in props)
			{
				...
				else if (type == typeof(DateTime))
				{
					...
					else
					{
						// try parsing instead
					    long seconds;
                        if (Int64.TryParse(value.AsString(), out seconds))
                            dt = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc).AddSeconds(seconds).ToLocalTime();
                        else
